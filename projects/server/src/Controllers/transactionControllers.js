        ...
        try {
            ...
            if (req.dataUser.idUser) {
                ...
                await Promise.all(addQuery.map(async (valArray) => {
                    contents = await dbQuery(valArray);
                    console.log(`contents`, contents.insertId);
                    console.log(`contents`, contents);

                    newestInsertId.push(contents.insertId)
                }));
                ...
                let addDetailQuery = [];
                if (newestInsertId.length >= 1) {
                    newestInsertId.forEach((valNewIdTransaction) => {
                        getAllCart.forEach((valAllCart) => {
                            req.body.arrayIdCart.forEach((valArrayIdCart) => {
                                if (valAllCart.idCart == valArrayIdCart) {
                                    if (addDetailQuery.findIndex((valNewQuery)=>{ valNewQuery.idTransaction == valNewIdTransaction})==-1){

                                        addDetailQuery.push(`insert into transactionsdetail (idTransaction, idStock, idUser, productName, productPicture, stockType, purchaseQuantity, priceSale, subTotal) values (${dbConf.escape(valNewIdTransaction)}, ${dbConf.escape(valAllCart.idStock)}, ${dbConf.escape(req.dataUser.idUser)}, ${dbConf.escape(valAllCart.productName)},${dbConf.escape(valAllCart.productPicture)},${dbConf.escape(valAllCart.stockType)},${dbConf.escape(valAllCart.cartQuantity)},${dbConf.escape(valAllCart.priceSale)},${dbConf.escape(valAllCart.subTotal)});`)
                ...
                await Promise.all(addDetailQuery.map(async (valArrayDetail) => {
                    ...
                    console.log(`contents2`, contents2.insertId);
                }));
