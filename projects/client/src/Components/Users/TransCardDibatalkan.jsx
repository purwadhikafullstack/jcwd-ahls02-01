...
import { getUserDibatalkanAction, getUserFilterDibatalkanAction } from "../../Redux/Actions/transactionActions";
import {
    Box,
    Divider,
    VStack,
...
const TransCardDibatalkanComponent = (props) => {
    ...
    const getArrayFilteredSortedTransaction = () => {
        dispatch(getUserFilterDibatalkanAction(props.query))
    }

    const getPaginatedTransaction = (page = 0) => {
        if (props.query.length == 0) {
            dispatch(getUserDibatalkanAction(page + 1))
        }
    }
...
export default TransCardDibatalkanComponent;